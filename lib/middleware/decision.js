...
module.exports = function descision(server, options, parse) {
	...
	if (!server)
		throw new Error('OAuth 2.0 descision middleware requires a server instance.');

	var cancelField = options.cancelField || 'cancel', userProperty = options.userProperty || 'user', key = options.sessionKey || 'authorize';

	return function descision(req, res, next) {
		if (!req.session) {
			return next(new Error('OAuth 2.0 server requires session support.'));
		}
		if (!req.session[key]) {
			return next(new Error('Invalid OAuth 2.0 session key.'));
		}
		if (!req.body) {
			return next(new Error('OAuth 2.0 server requires body parsing.'));
		}
		if (!req.oauth2) {
			return next(new Error('OAuth 2.0 transaction not found.'));
		}
